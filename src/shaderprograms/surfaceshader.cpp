...
SurfaceShader::SurfaceShader(GLWidget277 *context)
...
void SurfaceShader::setupMemberVars()
{
    ...
    unifEyePos = context->glGetUniformLocation(prog, "u_Eye");//new
...
void SurfaceShader::setEyePos(glm::vec4 p){
    useMe();

    if(unifEyePos != -1){
        context->glUniform4fv(unifEyePos, 1, &p[0]);//new
    }
